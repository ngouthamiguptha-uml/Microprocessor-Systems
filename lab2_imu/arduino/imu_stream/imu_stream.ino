...
void readAndSendIMU() {
  ...
  // Part A: burst-read 14 bytes starting at ACCEL_XOUT_H
  Wire.beginTransmission(MPU6050_ADDR);
  Wire.write(REG_ACCEL_XOUT_H);
  Wire.endTransmission(false);  // repeated START, keep bus
  ...
  // Part B: combine bytes into signed 16-bit values
  int16_t rawAX = (int16_t)((buf[0]  << 8) | buf[1]);
  ...
  // Part C: convert to physical units
