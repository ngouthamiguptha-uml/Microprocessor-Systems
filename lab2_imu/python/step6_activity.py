...
class ActivityDetector:
    ...
    def __init__(self) -> None:
        ...
        self._recent_dev: collections.deque = collections.deque(
            maxlen=max(STAT_WINDOW, SHAKE_COUNT)
        )
    ...
    def _classify(self) -> ActivityState:
        ...
        if len(devs) < min(STAT_WINDOW, SHAKE_COUNT):
            return self.state

        # SHAKING: several recent samples all have large deviation from 1g
        if len(devs) >= SHAKE_COUNT:
            recent_shake = devs[-SHAKE_COUNT:]
            if all(d >= THRESH_SHAKING for d in recent_shake):
                return ActivityState.SHAKING

        # STATIONARY: recent samples all stay close to 1g
        if len(devs) >= STAT_WINDOW:
            recent_stat = devs[-STAT_WINDOW:]
            if all(d < THRESH_MOVING for d in recent_stat):
                return ActivityState.STATIONARY

        # Otherwise it is moving, but not violently enough to be shaking
